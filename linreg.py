...
def loss_ridge(X, y, B, lmbda):
    return np.dot(np.transpose(y - np.dot(X, B)), y - np.dot(X, B)) + lmbda * np.dot(
        np.transpose(B), B
    )
...
def minimize(
    ...
    for i in range(max_iter):
        ...
        B = B - eta / (h + eps) ** 0.5 * loss_gradient(X, y, B, lmbda)
...
class LinearRegression621:
    ...
    def fit(self, X, y):
        self.B = minimize(X, y, loss_gradient, self.eta,
...
class LogisticRegression621:
    ...
    def fit(self, X, y):
        self.B = minimize(
            X, y, log_likelihood_gradient, self.eta, self.lmbda, self.max_iter
        )
...
class RidgeRegression621:
    ...
    def fit(self, X, y):
        ...
        self.B = minimize(
            X, y, loss_gradient_ridge, self.eta, self.lmbda, self.max_iter, self.addB0
        )
        B0 = np.mean(y)
